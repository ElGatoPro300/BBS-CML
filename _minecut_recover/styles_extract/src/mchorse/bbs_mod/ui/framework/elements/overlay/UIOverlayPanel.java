...
public class UIOverlayPanel extends UIElement
{
    ...
    protected void beginOpenTransition(UIContext context, float transition)
    {
        if (transition < 1.0F)
        {
            ...
            context.render.batcher.getContext().method_51448().method_46416(cx, cy, 0.0F);
            context.render.batcher.getContext().method_51448().method_22905(scale, scale, 1.0F);
            context.render.batcher.getContext().method_51448().method_46416(-cx, -cy, 0.0F);
        }
    }
    ...
    protected void renderBackground(UIContext context)
    {
        ...
        if (mchorse.bbs_mod.ui.framework.styles.UIStyle.isMinecut())
        {
            ...
            style.drawOverlayChrome(context.batcher, this.area);

            int headerH = 20;

            context.batcher.box(this.area.x, this.area.y, this.area.ex(), this.area.y + headerH, style.elevated());
            context.batcher.box(this.area.x, this.area.y + headerH - 1, this.area.ex(), this.area.y + headerH, style.borderSoft());
        }
        else
        {
            context.batcher.dropShadow(this.area.x, this.area.y, this.area.ex(), this.area.ey(), 10, 0x44000000, 0x00000000);
