    ...
    public UIFormCategory(FormCategory category, UIFormList list)
    {
        ...
        this.context((menu) ->
        {
            ...
            if (this.selected != null)
            {
                ...
                Collection<class_640> playerList = class_310.method_1551().method_1562().method_2880();
                ...
                if (playerList.size() > 1)
                {
                    menu.action(Icons.ARROW_RIGHT, UIKeys.FORMS_CATEGORIES_CONTEXT_SHARE_FORM, () ->
                    {
                        this.getContext().replaceContextMenu((newMenu) ->
                        {
                            ...
                            {
                                if (entry.method_2966().getId().equals(class_310.method_1551().field_1724.method_7334().getId()))
                                {
                                    continue;
                                }

                                newMenu.action(Icons.ARROW_RIGHT, IKey.constant(entry.method_2966().getName()), () ->
                                {
                                    ClientNetwork.sendSharedForm(this.selected, entry.method_2966().getId());
                                });
                            }
                        });
                    });
    ...
    public void render(UIContext context)
    {
        ...
        if (!forms.isEmpty() && this.category.visible.get())
        {
            for (Form form : forms)
            {
                ...
                boolean cellHover = context.mouseX >= cx && context.mouseX < cx + CELL_WIDTH
                    && context.mouseY >= cy && context.mouseY < cy + CELL_HEIGHT;

                mchorse.bbs_mod.ui.framework.styles.UIStyle.active().drawFormCell(
                    context.batcher, cx, cy, CELL_WIDTH, CELL_HEIGHT, isSelected, cellHover && !isSelected
                );

                boolean minecut = mchorse.bbs_mod.ui.framework.styles.UIStyle.isMinecut();
                int thumbBottom = minecut ? cy + CELL_HEIGHT - 12 : cy + CELL_HEIGHT;

                this.list.renderFormThumbnail(context, form, cx, cy, cx + CELL_WIDTH, thumbBottom);
                ...
                if (minecut)
                {
                    ...
                    String limited = context.batcher.getFont().limitToWidth(name, CELL_WIDTH - 4);
                    mchorse.bbs_mod.ui.framework.styles.UIStyle style =
                        mchorse.bbs_mod.ui.framework.styles.UIStyle.active();
                    int color = isSelected ? style.accent() : style.textDim();

                    context.batcher.textShadow(limited, cx + 2, cy + CELL_HEIGHT - 11, color);
                }
