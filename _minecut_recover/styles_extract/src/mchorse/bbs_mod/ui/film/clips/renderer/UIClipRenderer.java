...
public class UIClipRenderer <T extends Clip> implements IUIClipRenderer<T>
{
    ...
    private void renderEnvelopesKeyframes(class_287 builder, Matrix4f matrix, KeyframeChannel<Double> channel, int duration, int x1, int y1, int x2, int y2)
    {
        ...
        for (Keyframe<Double> keyframe : channel.getKeyframes())
        {
            if (prevKeyframe != null)
            {
                ...
                builder.method_22918(matrix, prevPoint.x, y2, 0F).method_39415(c);
                builder.method_22918(matrix, point.x, point.y, 0F).method_39415(c);
                builder.method_22918(matrix, prevPoint.x, prevPoint.y, 0F).method_39415(c);

                builder.method_22918(matrix, point.x, y2, 0F).method_39415(c);
                builder.method_22918(matrix, point.x, point.y, 0F).method_39415(c);
                builder.method_22918(matrix, prevPoint.x, y2, 0F).method_39415(c);
            }
        ...
        if (prevKeyframe != null && prevKeyframe.getTick() < duration)
        {
            ...
            builder.method_22918(matrix, point.x, y2, 0F).method_39415(c);
            builder.method_22918(matrix, x2, point.y, 0F).method_39415(c);
            builder.method_22918(matrix, point.x, point.y, 0F).method_39415(c);

            builder.method_22918(matrix, x2, y2, 0F).method_39415(c);
            builder.method_22918(matrix, x2, point.y, 0F).method_39415(c);
            builder.method_22918(matrix, point.x, y2, 0F).method_39415(c);
        }
    }
    ...
    protected void renderSimpleEnvelope(class_287 builder, Matrix4f matrix, Envelope envelope, int duration, int x1, int y1, int x2, int y2)
    {
        ...
        builder.method_22918(matrix, point.x, point.y, 0F).method_39415(c);
        builder.method_22918(matrix, previous.x, y2, 0F).method_39415(c);
        builder.method_22918(matrix, point.x, y2, 0F).method_39415(c);

        /* Third triangle */
        builder.method_22918(matrix, point.x, point.y, 0F).method_39415(c);
        builder.method_22918(matrix, previous.x, previous.y, 0F).method_39415(c);
        builder.method_22918(matrix, previous.x, y2, 0F).method_39415(c);
