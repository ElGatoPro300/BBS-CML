...
public class UIMobCaptureRecordOverlayPanel extends UIOverlayPanel
{
    ...
    public static void openInGame(Consumer<MobCaptureRecordingSetup> callback)
    {
        ...
        panel.onClose((event) -> class_310.method_1551().method_1507(null));
    ...
    private boolean isAllSelected()
    {
        ...
        for (MobCaptureAreaScanner.TypeBucket bucket : this.lastBuckets.values())
        {
            for (class_1297 entity : bucket.entities)
            {
                if (!this.setup.selectedEntityIds.contains(entity.method_5628()))
                {
                    return false;
                }
            }
    ...
    private boolean isAllVanilla()
    {
        ...
        for (MobCaptureAreaScanner.TypeBucket bucket : this.lastBuckets.values())
        {
            for (class_1297 entity : bucket.entities)
            {
                if (!this.setup.vanillaPlaybackEntityIds.contains(entity.method_5628()))
                {
                    return false;
                }
            }
    ...
    private void setAllSelected(boolean selected)
    {
        ...
        for (MobCaptureAreaScanner.TypeBucket bucket : this.lastBuckets.values())
        {
            ...
            for (class_1297 entity : bucket.entities)
            {
                this.setup.selectedEntityIds.add(entity.method_5628());
            }
        }
    }
    ...
    private void setAllVanilla(boolean enabled)
    {
        ...
        for (MobCaptureAreaScanner.TypeBucket bucket : this.lastBuckets.values())
        {
            ...
            for (class_1297 entity : bucket.entities)
            {
                this.setup.vanillaPlaybackEntityIds.add(entity.method_5628());
            }
        }
    }
    ...
    private void setTypeSelected(MobCaptureAreaScanner.TypeBucket bucket, boolean selected)
    {
        for (class_1297 entity : bucket.entities)
        {
            ...
            {
                this.setup.selectedEntityIds.remove(entity.method_5628());
            }
        }
    }
    ...
    private void setTypeVanilla(MobCaptureAreaScanner.TypeBucket bucket, boolean enabled)
    {
        ...
        else
        {
            ...
            for (class_1297 entity : bucket.entities)
            {
                this.setup.vanillaPlaybackEntityIds.remove(entity.method_5628());
            }
        }
    }
