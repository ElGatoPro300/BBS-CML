...
public class UIClips extends UIElement
{
    ...
    {
        if (type == null || duration < 1 || layer < 0 || tick < 0)
        {
            return;
        }

        Vector3i fit = this.checkSize(tick, layer, duration);
        ...
        this.addClip(clip, fit.x, fit.y, fit.z);
    }
    ...
    public int fromLayerY(int mouseY)
    {
        ...
        return Math.max(0, (bottom - mouseY) / LAYER_HEIGHT);
    }
    ...
    private void configureEmbeddedKeyframeEditor(UIKeyframeEditor keyframeEditor)
    {
        ...
        else
        {
            ...
            UIElement propertiesTarget;

            if (filmPanel.isMinecutFilmUi())
            {
                propertiesTarget = filmPanel.editArea;
            }
            else
            {
                propertiesTarget = filmPanel.shouldRedirectProperties()
    ...
    private void dragClips(int mouseX, int mouseY)
    {
        ...
        int currentLayer = this.fromLayerY(mouseY);
        int initialLayer = this.fromLayerY(this.initialY);

        if (currentLayer < 0 || initialLayer < 0)
        {
            return;
        }
        ...
        int dy = currentLayer - initialLayer;
    ...
    {
        int dir = 0;
        int guard = Math.abs(dx) + Math.abs(dy) + 4;
        ...
        while (this.collisionExists(others, data, dx, dy) && guard-- > 0)
        {
            ...
            if (!changed)
            {
                /* Stuck overlapping with no remaining delta — cancel the move. */
                return new int[]{0, 0};
            }
        }
