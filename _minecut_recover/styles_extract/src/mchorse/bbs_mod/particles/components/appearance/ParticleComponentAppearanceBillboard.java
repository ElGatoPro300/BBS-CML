...
public class ParticleComponentAppearanceBillboard extends ParticleComponentBase implements IComponentParticleRender
{
    ...
    public void renderGlowOverlay(ParticleEmitter emitter, Particle particle, class_287 builder, Matrix4f matrix, int overlay, float transition, Color glowColor)
    {
        this.renderWorld(emitter, class_290.field_1575, particle, builder, matrix, overlay, transition, glowColor);
    }

    public void renderUIGlowOverlay(Particle particle, class_287 builder, Matrix4f matrix, float transition, Color glowColor)
    {
    ...
    private void renderWorld(ParticleEmitter emitter, class_293 format, Particle particle, class_287 builder, Matrix4f matrix, int overlay, float transition, Color glowColor)
    {
    ...
    private void build(class_287 builder, class_293 format, Matrix4f matrix, ParticleEmitter emitter, Particle particle, int overlay)
    {
    ...
    private void buildGlow(class_287 builder, Matrix4f matrix, Particle particle, Color glowColor)
    {
        float u1 = this.u1 / (float) this.textureWidth;
        float u2 = this.u2 / (float) this.textureWidth;
        float v1 = this.v1 / (float) this.textureHeight;
    ...
    private void writeGlowVertex(class_287 builder, Matrix4f matrix, Vector4f vertex, float u, float v, Color color)
    {
        builder.method_22918(matrix, vertex.x, vertex.y, vertex.z)
            .method_22913(u, v)
            .method_22915(color.r, color.g, color.b, color.a);
    }
    ...
    private void writeVertex(class_287 builder, class_293 format, Matrix4f matrix, Vector4f vertex, float u, float v, int overlay, ParticleEmitter emitter, Particle particle)
    {
        ...
        else
        {
            ...
            builder.method_22918(matrix, vertex.x, vertex.y, vertex.z)
                ...
                .method_60803(this.light)
                .method_22914(this.n.x, this.n.y, this.n.z);
        }
    }
    ...
    {
        builder.method_22918(matrix, vertex.x, vertex.y, 0F)
            .method_22913(u, v)
            .method_22915(particle.r, particle.g, particle.b, particle.a);
    }
    ...
    public void calculateUVs(Particle particle, ParticleEmitter emitter, float transition)
    {
        ...
        else
        {
            ...
            int lightLevel = class_761.method_23794(emitter.world, blockPos);
