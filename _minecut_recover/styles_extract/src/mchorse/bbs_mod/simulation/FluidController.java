...
public class FluidController
{
    ...
    public void update(IEntity entity, FluidSimulation simulation, float scaleX, float scaleZ, float sensitivity, Matrix4f surfaceMatrixWorld, Iterable<BaseFilmController> controllers)
    {
        ...
        if (world != null)
        {
            ...
            class_1297 mcEntity = entity instanceof MCEntity ? ((MCEntity) entity).getMcEntity() : null;
            List<class_1297> entities = world.method_8335(mcEntity, box);

            for (class_1297 e : entities)
            {
                samples.add(new FluidSample(e.method_19538(), 0.1));
            }
        }
            ...
            for (IEntity replayEntity : controller.getEntities().values())
            {
                ...
                else
                {
                    for (var mapEntry : map.entrySet())
                    {
                        ...
                        samples.add(new FluidSample(new class_243(t.x, t.y, t.z), 0.1));
                    }
                }
            }
        }
        ...
        {
            class_243 p = sample.pos;
            Vector3f local = inverseSurface.transformPosition((float) p.field_1352, (float) p.field_1351, (float) p.field_1350, new Vector3f());
    ...
    private void updateModelInteraction(List<FluidSample> samples, ModelForm form, MatrixCache map, Matrix4f entityMatrix)
    {
        ...
        if (!BBSSettings.fluidRealisticModelInteraction.get())
        {
            ...
            samples.add(new FluidSample(new class_243(pos.x, pos.y, pos.z), 0.5));
                ...
                for (ModelMesh mesh : group.meshes)
                {
                    ...
                    for (int i = 0; i < vertexCount; i += 3)
                    {
                        ...
                        for (Vector3f p : points)
                        {
                            ...
                            samples.add(new FluidSample(new class_243(p.x, p.y, p.z), 0.1));
                        }
                    }
                }
            }
        ...
        else if (model instanceof BOBJModel)
        {
            ...
            for (int i = 0; i < vertexCount; i += stride)
            {
                ...
                {
                    samples.add(new FluidSample(new class_243(finalPos.x, finalPos.y, finalPos.z), 0.1));
                }
            }
        }
    }
    ...
    private void addCubeSamples(List<FluidSample> samples, ModelCube cube, Matrix4f transform)
    {
        ...
        for (Vector3f corner : corners)
        {
            ...
            samples.add(new FluidSample(new class_243(corner.x, corner.y, corner.z), 0.1));
        }
        ...
        for (Vector3f center : centers)
        {
            ...
            samples.add(new FluidSample(new class_243(center.x, center.y, center.z), 0.1));
        }
    }
    ...
    public static class FluidSample
    {
        ...
        public FluidSample(class_243 pos, double radius)
        {
            this.pos = pos;
            this.radius = radius;
        }
