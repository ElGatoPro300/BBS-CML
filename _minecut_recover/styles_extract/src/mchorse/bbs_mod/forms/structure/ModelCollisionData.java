...
public final class ModelCollisionData
{
    ...
    private ModelCollisionData(List<class_238> localBoxes, class_238 localBounds, Vector3f modelScale)
    {
        this(localBoxes, localBounds, modelScale, null);
    }

    private ModelCollisionData(List<class_238> localBoxes, class_238 localBounds, Vector3f modelScale, float[] skinnedVertices)
    {
    ...
    public static ModelCollisionData bakeFromPosedModel(Model model, Vector3f modelScale)
    {
        if (model == null)
        {
            return new ModelCollisionData(List.of(), new class_238(0, 0, 0, 0, 0, 0), modelScale != null ? modelScale : new Vector3f(1F));
        }
    ...
    public static ModelCollisionData bakeFromSkinnedBobj(BOBJLoader.CompiledData mesh, BOBJArmature armature, Vector3f modelScale)
    {
        ...
        {
            return new ModelCollisionData(List.of(), new class_238(0, 0, 0, 0, 0, 0), scale);
        }
        ...
        class_238 localBounds = aabbFromPosData(vertices);

        if (localBounds == null)
        {
            localBounds = new class_238(0, 0, 0, 0, 0, 0);
        }
    ...
    private static ModelCollisionData bakeStatic(ModelForm form)
    {
        ...
        {
            return new ModelCollisionData(List.of(), new class_238(0, 0, 0, 0, 0, 0), new Vector3f(1F));
        }
    ...
    private static ModelCollisionData finishBoxes(List<class_238> boxes, Vector3f modelScale, double surfaceSnug)
    {
        if (boxes.isEmpty())
        {
            return new ModelCollisionData(List.of(), new class_238(0, 0, 0, 0, 0, 0), modelScale);
        }

        List<class_238> snug = new ArrayList<>(boxes.size());

        for (class_238 box : boxes)
        {
            class_238 fitted = snugTop(box, surfaceSnug);
     ...
     */
    private static class_238 snugTop(class_238 box, double surfaceSnug)
    {
        if (box == null)
        {
            return null;
        ...
        return new class_238(box.field_1323, box.field_1322, box.field_1321, box.field_1320, box.field_1325 - pull, box.field_1324);
    }
    ...
    private static CachedModel loadCachedModel(String modelId)
    {
        ...
        List<class_238> fallback = new ArrayList<>();

        if (source.mesh != null)
        {
            class_238 box = aabbFromPosData(source.mesh.posData);
     ...
     */
    public static List<class_238> buildWorldSpaceBobjSlabs(float[] localVertices, Matrix4f worldMatrix)
    {
        ...
        List<class_238> boxes = new ArrayList<>(columns.size());
        ...
        for (Map.Entry<Long, float[]> entry : columns.entrySet())
        {
            ...
            boxes.add(new class_238(minX, bottom, minZ, maxX, top, maxZ));
        }
    ...
    private static class_238 aabbFromPosData(float[] posData)
    {
        if (posData == null || posData.length < 3)
        {
            return null;
    ...
    private static void collectCubicBoxes(Model model, List<class_238> boxes)
    {
    ...
    private static void walkGroup(ModelGroup group, Matrix4f parent, List<class_238> boxes)
    {
        ...
        {
            class_238 box = meshBox(mesh, local);
     ...
     */
    private static void appendCubeBoxes(ModelCube cube, Matrix4f groupMatrix, List<class_238> boxes)
    {
    ...
    private static class_238 meshBox(ModelMesh mesh, Matrix4f groupMatrix)
    {
    ...
    private static class_238 transformAabb(float minX, float minY, float minZ, float maxX, float maxY, float maxZ, Matrix4f matrix)
    {
        ...
        return new class_238(outMinX, outMinY, outMinZ, outMaxX, outMaxY, outMaxZ);
    }
    ...
    private static class_238 unionBounds(List<class_238> boxes)
    {
    ...
    private static List<class_238> thinBoxes(List<class_238> boxes, int max)
    {
        if (boxes.size() <= max)
        {
            return boxes;
        ...
        List<class_238> thinned = new ArrayList<>(max);
    ...
    private static final class CachedModel
    {
        ...
        private final List<class_238> bobjBoxes;
        private final Vector3f modelScale;
        private final Pose parts;
        private final boolean empty;

        private CachedModel(Model model, BOBJArmature bobjArmature, BOBJLoader.CompiledData bobjMesh, List<class_238> bobjBoxes, Vector3f modelScale, Pose parts, boolean empty)
        {
            this.model = model;
