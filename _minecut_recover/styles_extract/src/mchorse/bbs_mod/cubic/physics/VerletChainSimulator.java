...
final class VerletChainSimulator
{
    ...
    static void step(class_1937 world, int age, float transition, IModel model, List<String> ids, SpringChainCompiler.CompiledChain chain, float pullStrengthMul, float dragValue, float springReturnValue, WindDef wind, Map<String, JointLimitConfig.JointLimit> jointLimits, Vector3f anchorPosition, Quaternionf anchorOrientation, Quaternionf parentRotation, Vector3f targetPosition, List<PivotFrame> chainFrames, VerletChainSnapshot state)
    {
        ...
        class_2338.class_2339 mutable = collisions ? new class_2338.class_2339() : null;
    ...
    static final class IntegrationPhase
    {
        ...
        static void execute(VerletChainSnapshot state, float dampMul, float gravityX, float gravityY, float gravityZ, boolean hasWind, Vector3f windDir, float windMagnitude, WindDef wind, float filmTime, Vector3f windVec, float gravityScale, boolean collisions, class_1937 world, class_2338.class_2339 mutable, float radius, Vector3f vel)
        {
            VerletChainSnapshot.Particle[] particles = state.particles;
        ...
        private static void clampTunnelStep(class_1937 world, class_2338.class_2339 mutable, Vector3f p, Vector3f prev, float radius)
        {
            float dx = p.x - prev.x;
            float dy = p.y - prev.y;
            float dz = p.z - prev.z;
            ...
            int minBX = class_3532.method_15375(Math.min(prev.x, p.x) - radius);
            int minBY = class_3532.method_15375(Math.min(prev.y, p.y) - radius);
            int minBZ = class_3532.method_15375(Math.min(prev.z, p.z) - radius);
            int maxBX = class_3532.method_15375(Math.max(prev.x, p.x) + radius);
            int maxBY = class_3532.method_15375(Math.max(prev.y, p.y) + radius);
            int maxBZ = class_3532.method_15375(Math.max(prev.z, p.z) + radius);
    ...
    static final class CollisionPhase
    {
        ...
        static void execute(class_1937 world, VerletChainSnapshot state, Vector3f target, int last, float radius)
        {
