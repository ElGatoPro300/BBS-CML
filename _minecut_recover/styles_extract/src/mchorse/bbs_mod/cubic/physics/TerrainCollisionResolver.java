...
public final class TerrainCollisionResolver
{
    ...
    public static void resolve(class_1937 world, Vector3f[] pos, Vector3f[] prev, int from, int to, float radius, float friction)
    {
        ...
        float f = class_3532.method_15363(friction, 0F, 1F);
    ...
    private static void gatherNearbyBoxes(class_1937 world, class_2338.class_2339 mutable, List<float[]> boxes, Vector3f pos, Vector3f prev, float radius)
    {
        boxes.clear();
        ...
        collectBoxesInAabb(world, mutable, boxes,
            class_3532.method_15375(minX - radius), class_3532.method_15375(minY - radius), class_3532.method_15375(minZ - radius),
            class_3532.method_15375(maxX + radius), class_3532.method_15375(maxY + radius), class_3532.method_15375(maxZ + radius));
    }

    private static void gatherSegmentBoxes(class_1937 world, class_2338.class_2339 mutable, List<float[]> boxes, Vector3f a, Vector3f b, float radius)
    {
        boxes.clear();

        collectBoxesInAabb(world, mutable, boxes,
            class_3532.method_15375(Math.min(a.x, b.x) - radius), class_3532.method_15375(Math.min(a.y, b.y) - radius), class_3532.method_15375(Math.min(a.z, b.z) - radius),
            class_3532.method_15375(Math.max(a.x, b.x) + radius), class_3532.method_15375(Math.max(a.y, b.y) + radius), class_3532.method_15375(Math.max(a.z, b.z) + radius));
    }

    private static void collectBoxesInAabb(class_1937 world, class_2338.class_2339 mutable, List<float[]> boxes, int bx1, int by1, int bz1, int bx2, int by2, int bz2)
    {
        for (int x = bx1; x <= bx2; x++)
        {
            for (int y = by1; y <= by2; y++)
            {
                for (int z = bz1; z <= bz2; z++)
                {
                    ...
                    if (state.method_26234(world, mutable))
                    {
                        boxes.add(new float[] {x, y, z, x + 1F, y + 1F, z + 1F});
                        continue;
                    }

                    class_265 shape = state.method_26194(world, mutable, class_3726.method_16194());
                    ...
                    {
                        boxes.add(new float[] {
                            (float) (x + box.field_1323), (float) (y + box.field_1322), (float) (z + box.field_1321),
                            (float) (x + box.field_1320), (float) (y + box.field_1325), (float) (z + box.field_1324)
                        });
                    }
                }
            }
    ...
    public static boolean hasFullCubeInAabb(class_1937 world, class_2338.class_2339 mutable, int minBX, int minBY, int minBZ, int maxBX, int maxBY, int maxBZ)
    {
        if (world == null)
        {
            return false;
        ...
        for (int x = minBX; x <= maxBX; x++)
        {
            for (int y = minBY; y <= maxBY; y++)
            {
                for (int z = minBZ; z <= maxBZ; z++)
                {
                    ...
                    class_2680 block = world.method_8320(mutable);
                    ...
                    if (block.method_26234(world, mutable))
                    {
                        return true;
                    }

                    if (!block.method_26194(world, mutable, class_3726.method_16194()).method_1110())
                    {
                        return true;
                    }
                }
