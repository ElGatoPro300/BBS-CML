...
public class PhysBoneRuntime
{
    ...
    public static void update(IEntity entity, IModelInstance modelInstance, Map<String, PhysBoneState> physStates)
    {
        ...
        float bodyTurn = sanitizeRotation(class_3532.method_15393(bodyYaw - prevBodyYaw));
        float yawSin = class_3532.method_15374(MathUtils.toRad(bodyYaw));
        float yawCos = class_3532.method_15362(MathUtils.toRad(bodyYaw));
        ...
        for (PhysBoneDefinition definition : sorted)
        {
            ...
            float targetYaw = class_3532.method_15363(swayYaw, -safeAngle, safeAngle);
            float targetPitch = class_3532.method_15363(swayPitch, -safeAngle, safeAngle);
                ...
                if (anchorPivot != null && bonePivot != null)
                {
                    ...
                    if (toAnchor.lengthSquared() > 0.0001F)
                    {
                        ...
                        if (parentRot != null)
                        {
                            ...
                            targetYaw = class_3532.method_16439(0.35F, targetYaw, anchorYaw);
                            targetPitch = class_3532.method_16439(0.35F, targetPitch, anchorPitch);
                        }
                    }
                }
            }
            ...
            if (definition.affectRoll)
            {
                ...
                targetRoll = class_3532.method_15363(rollSway, -safeAngle * 0.7F, safeAngle * 0.7F);
            }
    ...
    private static void integrateSpring(PhysBoneState state, PhysBoneDefinition definition, float targetYaw, float targetPitch, float targetRoll, float stiffness, float damping, float gravity, float dt, float safeAngle)
    {
        ...
        for (int step = 0; step < subSteps; step++)
        {
            ...
            state.yaw = class_3532.method_15363(state.yaw + state.yawVelocity * subDt, limitMinYaw, limitMaxYaw);
            ...
            state.pitch = class_3532.method_15363(state.pitch + state.pitchVelocity * subDt, limitMinPitch, limitMaxPitch);

            /* Roll spring */
            float rollForce = (targetRoll - state.roll) * stiffness - state.rollVelocity * damping;
            state.rollVelocity += rollForce * subDt;
            state.roll = class_3532.method_15363(state.roll + state.rollVelocity * subDt, limitMinRoll, limitMaxRoll);
        }

        state.yaw = class_3532.method_15363(state.yaw, limitMinYaw, limitMaxYaw);
        state.pitch = class_3532.method_15363(state.pitch, limitMinPitch, limitMaxPitch);
        state.roll = class_3532.method_15363(state.roll, limitMinRoll, limitMaxRoll);
    }
        ...
        if (model instanceof Model)
        {
            ...
            for (ModelGroup group : model.getAllGroups())
            {
                if (torso != null && isSoftBodyCandidate(group))
                {
                    ...
                    limits.put(group.id, class_3532.method_15363(limit, 12F, 45F));
                }
                else if (head != null && isHairCandidate(group))
                {
                    ...
                    if (headDepth >= 0)
                    {
                        ...
                        limits.put(group.id, class_3532.method_15363(limit, 8F, 28F));
                    }
                }
                else if (torso != null && isBodyCollisionCandidate(group, torso))
                {
                    ...
                    limits.put(group.id, class_3532.method_15363(limit, 8F, 24F));
                }
            }
        }
    ...
    private static Vector2f getParentPoseDelta(IEntity entity, String bone, Map<String, ModelGroup> groupsById, Map<String, BOBJBone> bobjBones, PhysBoneState state)
    {
        ...
        float yawTurn = class_3532.method_15393(parentRotation.x - state.prevParentYaw);
        float pitchTurn = class_3532.method_15393(parentRotation.y - state.prevParentPitch);
    ...
    private static float computeGravityTilt(float axis, float downY)
    {
        ...
        return class_3532.method_15363(tilt * 1.4F, -55F, 55F);
    }
    ...
    private static float estimateModelRootRadius(ModelGroup root, Map<String, ModelGroup> groupsById, Set<String> dynamicBones, float fallback, float min, float max)
    {
        ...
        return class_3532.method_15363(radius, min, max);
    }
    ...
    private static void solveCollision(IEntity entity, IModel model, String bone, PhysBoneState state, float prevYaw, float prevPitch, Map<String, PhysBoneState> physStates, Map<String, ModelGroup> groupsById, Map<String, BOBJBone> bobjBones, List<CollisionSphere> collisionSpheres, float collisionRadius)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            float yaw = class_3532.method_16439(mid, prevYaw, state.yaw);
            float pitch = class_3532.method_16439(mid, prevPitch, state.pitch);
        ...
        state.yaw = class_3532.method_16439(low, prevYaw, state.yaw);
        state.pitch = class_3532.method_16439(low, prevPitch, state.pitch);
    ...
    private static boolean collides(IEntity entity, IModel model, String bone, String parent, float yaw, float pitch, Map<String, PhysBoneState> physStates, Map<String, ModelGroup> groupsById, Map<String, BOBJBone> bobjBones, List<CollisionSphere> collisionSpheres, float collisionRadius)
    {
        ...
        class_1937 world = entity == null ? null : entity.getWorld();
        ...
        class_243 worldParent = toWorldSpace(entity, parentPivot);
        class_243 worldTip = toWorldSpace(entity, tip);
    ...
    private static class_243 toWorldSpace(IEntity entity, Vector3f local)
    {
        float yaw = entity.getBodyYaw() + 180F;
        double sin = class_3532.method_15374(MathUtils.toRad(yaw));
        double cos = class_3532.method_15362(MathUtils.toRad(yaw));
        double x = (local.x * cos - local.z * sin) / 16D;
        double z = (local.x * sin + local.z * cos) / 16D;
        double y = local.y / 16D;

        return new class_243(entity.getX() + x, entity.getY() + y, entity.getZ() + z);
    }
    ...
    private static boolean hasEnvironmentCollision(class_1937 world, class_243 from, class_243 to, float radius)
    {
        if (world == null || from == null || to == null || from.method_1025(to) < 0.00001D)
        {
            return false;
        }

        class_243 delta = to.method_1020(from);
        class_243 adjusted = class_1297.method_20736(null, delta, new class_238(
            from.field_1352 - radius,
            from.field_1351 - radius,
            from.field_1350 - radius,
            from.field_1352 + radius,
            from.field_1351 + radius,
            from.field_1350 + radius
        ), world, Collections.emptyList());

        if (adjusted.method_1025(delta) > 0.00000025D)
        {
            return true;
        }

        double length = Math.sqrt(from.method_1025(to));
        int steps = class_3532.method_15340((int) Math.ceil(length / Math.max(0.02D, radius * 0.45D)), 3, 26);

        for (int i = 0; i <= steps; i++)
        {
            double factor = i / (double) steps;
            class_243 point = from.method_1019(delta.method_1021(factor));
        ...
        class_239 result = world.method_17742(new class_3959(
            from,
            to,
    ...
    private static boolean intersectsWorldCollision(class_1937 world, class_243 point, float radius)
    {
        int minX = class_3532.method_15357(point.field_1352 - radius) - 1;
        int maxX = class_3532.method_15357(point.field_1352 + radius) + 1;
        int minY = class_3532.method_15357(point.field_1351 - radius) - 1;
        int maxY = class_3532.method_15357(point.field_1351 + radius) + 1;
        int minZ = class_3532.method_15357(point.field_1350 - radius) - 1;
        int maxZ = class_3532.method_15357(point.field_1350 + radius) + 1;
        class_3726 shape = class_3726.method_16194();
        ...
        for (int x = minX; x <= maxX; x++)
        {
            for (int y = minY; y <= maxY; y++)
            {
                for (int z = minZ; z <= maxZ; z++)
                {
                    ...
                    class_265 collision = state.method_26194(world, pos, shape);

                    if (collision == null || collision.method_1110())
                    {
                        continue;
                    }

                    for (class_238 box : collision.method_1090())
                    {
                        class_238 worldBox = box.method_996(pos);
                        double nx = class_3532.method_15350(point.field_1352, worldBox.field_1323, worldBox.field_1320);
                        double ny = class_3532.method_15350(point.field_1351, worldBox.field_1322, worldBox.field_1325);
                        double nz = class_3532.method_15350(point.field_1350, worldBox.field_1321, worldBox.field_1324);
                        double dx = point.field_1352 - nx;
                        double dy = point.field_1351 - ny;
                        double dz = point.field_1350 - nz;
