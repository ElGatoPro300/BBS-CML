...
public final class ActorReplayStateSync
{
    ...
    public static void applyFromKeyframes(ReplayKeyframes keyframes, float tick, LivingEntity actor, boolean mounted, boolean advanceLimbs)
    {
        ...
        boolean applyUseFlags = !(actor instanceof Player);

        actor.setLivingEntityFlag(1, (usingItem && applyUseFlags) || blocking);
        actor.setLivingEntityFlag(2, offHand && usingItem && applyUseFlags);
     ...
     * <p>
     * First-person playback binds the real {@link Player} as the actor body; that
