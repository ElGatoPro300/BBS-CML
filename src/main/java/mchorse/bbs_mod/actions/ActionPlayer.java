...
public class ActionPlayer
{
    ...
    {
        this.applyActionsFiltered(true, this.tick - 1F, this.tick);
    }
    ...
    private int applyActionsUpTo(float filmTime)
    {
        ...
        float curr = filmTime;
        int fired = this.applyActionsFiltered(false, prev, curr);
    ...
    private int applyActionsFiltered(boolean skipCombatClips, float prev, float curr)
    {
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            for (Clip clip : replay.actions.getClipsCrossing(prev, curr))
            {
                ...
                {
                    actionClip.apply(actor, fakePlayer, this.film, replay, (int) curr);
                    fired++;
                }
            }
        }
