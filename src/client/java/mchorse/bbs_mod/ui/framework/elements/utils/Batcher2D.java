...
public class Batcher2D
{
    ...
    private static RenderPipeline.Builder guiColorBuilder(String name, PrimitiveTopology topology)
    {
    ...
    private static void flush(BufferBuilder builder, RenderType renderLayer)
    {
        ...
        if (built != null)
        {
            Draw.drawMeshData(built, renderLayer);
            built.close();
        }
    }
    ...
    public void texturedBox(int texture, int color, float x, float y, float w, float h, float u1, float v1, float u2, float v2, int textureW, int textureH, boolean linear)
    {
