...
public class VideoFormRenderer extends FormRenderer<VideoForm> implements ITickable
{
        ...
        boolean invertY, boolean modelRenderer, FormRenderingContext deferContext)
    {
        ...
        Runnable draw = () ->
        {
            ...
            {
                this.drawSolidFront(positionMatrix, tintSnapshot, localQuad, finalPickShader);
            }
            else if (staticPreview)
            {
    ...
    private void drawVideoFront(Matrix4f matrix, Color tint, Quad quad, int textureId, boolean linear)
    {
        ...
        BillboardRenderLayers.draw(buffer.buildOrThrow(), id, linear, false, true, false);
    }
    ...
    private void drawSolidFront(Matrix4f matrix, Color color, Quad quad, GlProgram shader)
    {
        ...
        {
            Draw.getPositionColorLayer().draw(buffer.buildOrThrow());
        }
    }

    private void texCol(BufferBuilder buffer, Matrix4f matrix, float x, float y, float z, float u, float v, Color color)
    {
        buffer.addVertex(matrix, x, y, z).setUv(u, v).setColor(color.r, color.g, color.b, color.a);
    }
