...
/**
 * Isolated backend for WaterMedia integration.
 * Loaded ONLY when the "watermedia" mod is present in Fabric.
 */
class WaterMediaBackend
{
    ...
    {
        VideoRenderer.FrameInfo frame = prepareFrame(path, position, playing, loops, volume);
    ...
    public static VideoRenderer.FrameInfo prepareFrame(String path, long tickPosition, boolean playing, boolean loops, int volume)
    {
        return prepareFrame(path, tickPosition, playing, loops, volume, false, false, 1F, 0, MediaPlayer.LodLevel.MAX);
    }

    private static VideoRenderer.FrameInfo prepareFrame(String path, long tickPosition, boolean playing, boolean loops, int volume, boolean formMode)
    {
        return prepareFrame(path, tickPosition, playing, loops, volume, formMode, false, 1F, 0, MediaPlayer.LodLevel.MAX);
    }

    public static VideoRenderer.FrameInfo prepareFormFrame(String path, long tickPosition, boolean loops, float distanceSq, int maxLongSide, boolean playing, boolean filmSync, float speed, int baseVolume)
    {
        String resolved = VideoRenderer.resolveVideoPath(path);
    ...
    public static VideoRenderer.FrameInfo peekFormFrame(String path)
    {
        String resolved = VideoRenderer.resolveVideoPath(path);
        ...
        return new VideoRenderer.FrameInfo(texture, width, height);
    }

    public static VideoRenderer.FrameInfo ensureFormStillFrame(String path, long tickPosition, boolean loops)
    {
        VideoRenderer.FrameInfo peeked = peekFormFrame(path);
    ...
    private static VideoRenderer.FrameInfo prepareFrame(String path, long tickPosition, boolean playing, boolean loops, int volume, boolean formMode, boolean filmSync, float speed, int maxLongSide, MediaPlayer.LodLevel lod)
    {
        String resolved = VideoRenderer.resolveVideoPath(path);
