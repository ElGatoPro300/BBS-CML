...
public class VideoRenderer
{
    ...
    public static void render(PoseStack stack, Batcher2D batcher, String path, long position, boolean playing, int volume, int x, int y, int w, int h, float opacity, int cropX, int cropY, int cropWidth, int cropHeight, boolean loops)
    {
        ...
        buffer.addVertex(matrix, drawX, drawY + drawH, 0).setUv(u0, v1).setColor(color);
        buffer.addVertex(matrix, drawX + drawW, drawY + drawH, 0).setUv(u1, v1).setColor(color);
        buffer.addVertex(matrix, drawX + drawW, drawY, 0).setUv(u1, v0).setColor(color);
        buffer.addVertex(matrix, drawX, drawY, 0).setUv(u0, v0).setColor(color);

        BillboardRenderLayers.draw(buffer.buildOrThrow(), id, false, false, false, false);
    }
        ...
        if (wrapper == null)
        {
            ...
            try
            {
                ...
                player = MediaAPI.createPlayer(
                    mrl,
                    () -> MediaAPI.glEngine(((MinecraftClientInvoker) client).bbs$getRunningThread(), client),
