...
/**
 * Minimal physics overlay, the same drawing language as the IK one: each chain is
 * a run of wires through joint markers, the pinned root picked out in a warm accent
 * and the simulated tip in a cool one, with the pin target bridged to the chain's
 * end by a dashed relationship line. Every part's look comes from
 * {@link ValuePhysicsDebug} ({@code BBSSettings.physicsDebug}), which also holds the
 * overlay's toggle.
 *
 * <p>It reads the model-local pivot frames the renderer already produced — physics
 * is applied to the rig before this runs, so the frames are the settled chain. Only
 * the virtual tip (the solver simulates one segment past the last bone) is
 * reconstructed, from that bone's solved rotation.
 *
 * <p>{@link #renderStencil} mirrors the pin-target markers into the picking pass so
 * clicking one selects its (often mesh-less) bone, at the same shape and size as the
 * visible marker; hidden markers are not pickable.
 *
 * <p>When wind is configured, an arrow at each free point shows the force it
 * currently feels, sampled from the same noise the solver uses — so the arrows pulse
 * with the gusts and vary along a chain exactly as the ripple does.
 */
public final class ModelPhysicsDebug
{
    ...
    public static void render(PoseStack stack, IModel model, MapType physicsData, int age, String selectedRoot)
    {
        ...
        SpringChainCompiler.Compiled compiled = SpringChainCompiler.getFromData(model, physicsData);
        ...
        /* Wind is one field for the whole model: resolve its world direction and base
         * magnitude once, plus the inverse of the draw matrix so a world-space force can
         * be carried back into the overlay's local drawing space for each arrow. */
        Vector3f windDir = new Vector3f();
        float windMagnitude = config.wind.visible.get() ? EnvironmentalForces.prepareWind(compiled.wind(), 1F, windDir) : 0F;
        Matrix4f matrix = new Matrix4f(stack.last().pose());
        Matrix4f inverse = windMagnitude > 0F ? new Matrix4f(matrix).invert() : null;

        for (SpringChainCompiler.CompiledChain chain : compiled.chains())
        {
    ...
    private static Map<String, PivotFrame> collectFrames(IModel model, SpringChainCompiler.Compiled compiled)
    {
    ...
    /**
     * Mirrors the pin targets into the picking pass: a pickable marker at each one,
     * registered under that bone. Must run after the model's bones are registered so
     * the marker ids fall right after them.
     */
    public static void renderStencil(PoseStack stack, IModel model, MapType physicsData, StencilMap stencilMap, Form form)
    {
        ...
        /* The id rides the VERTEX COLOUR and position_color multiplies it by ShaderColor,
         * so a tint left by the model pass would scale it into a different index and the
         * click would select another bone. Same guard the gizmo's pick pass uses. */
        BBSRendering.setShaderColor(1F, 1F, 1F, 1F);
        ...
        for (SpringChainCompiler.CompiledChain chain : compiled.chains())
        {
            ...
            Vector3f target = position(frames, chain.pinTarget());

            if (target != null)
            {
                pickMarker(builder, stack, stencilMap, form, config.attach, target, segmentUnit(chain.restLengths()), chain.pinTarget());
            }
        }
    ...
    private static List<Vector3f> chainPoints(IModel model, Map<String, PivotFrame> frames, SpringChainCompiler.CompiledChain chain)
    {
    ...
    private static void drawChain(PoseStack stack, IModel model, Map<String, PivotFrame> frames, SpringChainCompiler.CompiledChain chain, String selectedRoot, ValuePhysicsDebug config)
    {
        ...
        Vector3f target = chain.pinTarget() == null || chain.pinTarget().isEmpty() ? null : position(frames, chain.pinTarget());
        ...
        RenderType linesLayer = xray ? Draw.getPositionColorLinesNoDepthLayer() : Draw.getPositionColorLinesLayer();
        RenderType trisLayer = xray ? Draw.getPositionColorNoDepthLayer() : Draw.getPositionColorLayer();
        ...
        /* Solid geometry: the pinned root, joints, the simulated tip and the pin target,
         * plus the thick lines. */
    ...
    /**
     * Draws the wind arrow at each free point of a chain: the force it currently feels,
     * sampled from the same noise the solver uses, so the arrows pulse with the gusts
     * and vary along the chain just like the ripple. The pinned root (point 0) feels no
     * wind and is skipped. Length is proportional to the force, scaled to the chain's
     * segment length.
     */
    private static void drawWind(PoseStack stack, IModel model, Map<String, PivotFrame> frames, SpringChainCompiler.CompiledChain chain, String selectedRoot, WindDef wind, Vector3f windDir, float windMagnitude, int age, Matrix4f matrix, Matrix4f inverse, ValuePhysicsDebug config)
    {
        List<Vector3f> pts = chainPoints(model, frames, chain);
        ...
        for (int i = 1; i < pts.size(); i++)
        {
            ...
            /* Sample in the solver's world space (carry the local point out through the
             * draw matrix), then carry the world-space force back into local space so the
             * arrow points the right way on screen. A model-relative wind is already
             * expressed in the model's own frame — this drawing space — so it skips the
             * round trip. */
            matrix.transformPosition(world.set(p));
            EnvironmentalForces.windForceAt(windDir, windMagnitude, wind, age, world, force);

            if (!wind.modelRelative())
            {
    ...
    /**
     * The solver simulates one virtual point past the last bone, and the last bone is
     * rotated toward it. Rebuilds that point from the bone's solved world rotation and
     * the same rest direction the rotation appliers used, so the drawn tip is exactly
     * where the bone is pointing.
     */
    private static Vector3f tipPosition(IModel model, List<String> ids, Map<String, PivotFrame> frames, float[] lengths)
    {
        ...
        Vector3f dir = SkeletonGeometryAdapter.tipRestDirectionLocal(model, ids);
