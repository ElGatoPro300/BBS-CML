...
import mchorse.bbs_mod.cubic.model.fbx.FBXNode;
import mchorse.bbs_mod.cubic.model.fbx.FBXParser;
import mchorse.bbs_mod.cubic.model.fbx.FBXToBOBJConverter;
...
public class FBXModelLoader implements IModelLoader
{
    ...
    public ModelInstance load(String id, ModelManager models, Link model, Collection<Link> links, MapType config)
    {
        ...
        String fbxResolvedTexture = null;

        try (InputStream stream = models.provider.getAsset(modelFBX))
        {
            FBXNode root = FBXParser.parse(stream);
            File fbxFile = models.provider.getFile(modelFBX);
            FBXToBOBJConverter converter = new FBXToBOBJConverter(root, fbxFile);

            BOBJLoader.BOBJData bobjData = converter.convert();
            fbxResolvedTexture = converter.resolvedTexturePath;

            if (modelTexture == null && fbxResolvedTexture != null)
            {
                modelTexture = Link.create(fbxResolvedTexture);
            }
            ...
            for (BOBJLoader.BOBJMesh mesh : bobjData.meshes)
            {
                ...
                {
                    BOBJLoader.CompiledData compiledData = BOBJLoader.compileMesh(bobjData, mesh);
                    compiledMeshes.add(compiledData);
                }
            }
