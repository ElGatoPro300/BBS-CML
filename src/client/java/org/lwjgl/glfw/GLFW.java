...
/**
 * Compatibility bridge for GLFW constants and utilities on Minecraft 26.3+ (which uses SDL3 / InputConstants).
 */
public class GLFW
{
    /* Keyboard keys */
    ...
    /* Cursor modes and standard cursors */
    ...
    public static String glfwGetKeyName(int keyCode, int scancode)
    {
        return null;
    }
    ...
    public static void glfwGetCursorPos(long window, double[] x, double[] y)
    {
        ...
        {
            if (x != null && x.length > 0) x[0] = mc.mouseHandler.xpos();
            if (y != null && y.length > 0) y[0] = mc.mouseHandler.ypos();
        }
    }
    ...
    public static void glfwSetClipboardString(long window, String string)
    {
        Minecraft mc = Minecraft.getInstance();

        if (mc != null && mc.keyboardHandler != null && string != null)
        {
            mc.keyboardHandler.setClipboard(string);
        }
    }

    public static void glfwSetClipboardString(long window, ByteBuffer buffer)
    {
        if (buffer != null)
        {
            byte[] bytes = new byte[buffer.remaining()];
            buffer.get(bytes);
            glfwSetClipboardString(window, new String(bytes, StandardCharsets.UTF_8).trim());
        }
    }
